    ...
    public function __invoke(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            if ($verifyTransaction->transaction->status === TransactionStatus::CANCELED->value) {
                session()->flash(
                    key: 'error',
                    value: __('Votre paiement a été annulé veuillez relancer pour soutenir Laravel Maroc, Merci.')
                );
            } else {
                ...
                session()->flash(
                    key: 'status',
                    value: __('Votre paiement a été pris en compte merci de soutenir Laravel Maroc.')
                );
            }
