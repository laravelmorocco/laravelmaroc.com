    ...
    <div class="isolate -mt-16 pt-14">
        <div class="absolute inset-0 -z-10 mx-0 max-w-none overflow-hidden">
            <div class="absolute left-1/2 top-0 ml-[-32rem] h-[32rem] w-[84.25rem]">
                ...
                <svg viewBox="0 0 1113 440" aria-hidden="true"
                    class="absolute left-1/2 top-0 ml-[-24rem] w-[70.5625rem] fill-green-500 blur-2xl opacity-30 logo-dark">
                    <path d="M.016 439.5s-9.5-300 434-300S882.516 20 882.516 20V0h230.004v439.5H.016Z" />
                </svg>
                <svg viewBox="0 0 1113 440" aria-hidden="true"
                    class="absolute left-1/2 top-0 ml-[-24rem] w-[70.5625rem] fill-yellow-50 blur-2xl opacity-50 logo-white">
                    <path d="M.016 439.5s-9.5-300 434-300S882.516 20 882.516 20V0h230.004v439.5H.016Z" />
                </svg>
            </div>
        </div>
        <div class="mx-auto max-w-3xl px-4 pb-16 pt-28 sm:pt-32 lg:pt-40">
            <div class="flex justify-center">
                <a href="{{ route('sponsors') }}"
                    class="inline-flex items-center p-1 pr-2 font-sans text-white bg-green-700 rounded-full sm:text-base lg:text-sm xl:text-base">
                    <span
                        class="px-3 py-0.5 text-white text-xs font-semibold leading-5 uppercase tracking-wide bg-flag-green rounded-full">
                        ⚡️ {{ __('Sponsor') }}
                    </span>
                    <span
                        class="ml-4 hidden text-sm sm:block">{{ __('Soutenez Laravel Maroc aujourd\'hui en sponsorisant') }}</span>
            ...
            <div class="mt-10 text-center">
                ...
                <div class="mt-10 sm:flex sm:items-center sm:justify-center gap-x-6">
                    ...
                    @endauth
                    <x-default-button :link="route('forum.index')"
                        class="w-full mt-3 text-base font-medium sm:mt-0 sm:ml-3 sm:shrink-0 sm:inline-flex sm:items-center sm:w-auto">
                        {{ __('Visiter le Forum') }}
                    </x-default-button>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 py-10 lg:py-12 xl:pb-14">
            ...
            <div class="mt-6 text-center lg:mt-10">
                <a class="text-sm leading-5 text-flag-green hover:text-green-600 hover:underline"
                    href="{{ route('sponsors') }}">
                    {{ __('Votre logo ici ?') }}
                </a>
            </div>
    ...
    <x-container class="px-4 mx-auto max-w-7xl">
        <div class="divide-y divide-skin-base">
            <div class="py-12 lg:py-20">
                <x-section-header title="Articles Populaires"
            ...
            @if ($latestThreads->isNotEmpty())
                <div class="py-12 lg:py-20">
                    <x-section-header title="On apprend aussi en aidant les autres"
                        ...
                        @foreach ($latestThreads as $thread)
                            <div>
                                <div class="flex items-center font-sans text-skin-base">
                                    <a href="{{ route('profile', $thread->user->username) }}"
                                        class="inline-flex items-center hover:underline">
                                        <x-user.avatar :user="$thread->user" class="h-6 w-6" container="mr-1.5"
                                            span="-right-1 -top-1 w-4 h-4 ring-1" />
                                        <span class="font-sans">{{ '@' . $thread->user->username }}</span>
                                    </a>
                                    <span class="inline-flex mx-1.5 space-x-1.5">
                                        <span>{{ __('a posé') }}</span>
                                        <time class="sr-only" datetime="{{ $thread->created_at }}"
                                            title="{{ $thread->last_posted_at->format('j M, Y \à H:i') }}">{{ $thread->last_posted_at->format('j M, Y \à H:i') }}</time>
            ...
            <div class="py-12 lg:py-20">
                <x-section-header title="Ou venez juste discuter"
                        ...
                        <div>
                            <div class="flex items-center font-sans text-sm text-skin-muted">
                                <a class="shrink-0" href="/user/{{ $discussion->user->username }}">
                                    <x-user.avatar :user="$discussion->user" class="h-6 w-6" container="mr-1.5"
                                        span="-right-1 -top-1 w-4 h-4 ring-1" />
                                </a>
                                <span class="pr-1 ml-2">{{ __('Posté par') }}</span>
                                <div class="flex items-center space-x-1">
                                    <a href="{{ route('profile', $discussion->user->username) }}"
                                        class="text-skin-inverted hover:underline">{{ $discussion->user->name }}</a>
                            ...
                            <div class="mt-3">
                                <a href="{{ route('discussions.show', $discussion) }}"
                                    class="text-base font-medium text-flag-green hover:underline hover:text-green-500">
            ...
            <div class="w-full h-full xl:grid xl:grid-cols-2">
                <div class="h-full xl:relative xl:col-start-2">
                    <img class="object-cover w-full h-full opacity-25 xl:absolute xl:inset-0"
                        src="{{ asset('/images/developer.jpg') }}" alt="Developer working on laptop" />
